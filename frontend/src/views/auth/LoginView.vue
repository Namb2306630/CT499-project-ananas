...
const canSubmit = computed(() => {
  return authUser.value.phone.trim() !== '' && authUser.value.password.trim() !== ''
})
const login = async () => {
  authStore.clearError()
  ...
  if (res?.code === 200) {
    ...
    const message =
      Object.values(authStore.error.errors)[0] || authStore.error.general || 'Lỗi đăng nhập!!!'
  ...
  <AuthFormLayout title="Đăng nhập">
    <form class="login-form" @submit.prevent="login">
      ...
      <div class="login-agreement">
        <p class="p-0 m-0">
          Bằng việc đăng nhập, bạn đã đồng ý với Ananas về
          <RouterLink> Điều khoảng dịch vụ </RouterLink> &
          <RouterLink>Chính sách bảo mật</RouterLink>
        </p>
      </div>
      ...
      <div class="register">
        <p class="p-0 m-0">
          Bạn chưa có tài khoản?<RouterLink :to="{ name: ROUTE_NAMES.REGISTER }">
            Đăng ký
          </RouterLink>
        </p>
      </div>
